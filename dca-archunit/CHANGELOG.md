...
- `DCA-LAY-004` also sees programmatic boundaries: a class outside the application layer and the outgoing adapters
  ...
  `TransactionBoundary` is reported, as an annotated one always was. Implementations of `TransactionBoundary` and
  the global and shared-kernel infrastructure packages (the composition root wiring the transaction manager and its
  plumbing) are the exempt sites. A bootstrap or web adapter that wraps its work in a transaction template turns red - move the
