...
class ContextMapRulesTest {
  ...
  private static final String GOOD = "dev.domaincentric.dca.archunit.fixtures.contextmap.good";
  private static final String BAD = "dev.domaincentric.dca.archunit.fixtures.contextmap.bad";
  ...
  Stream<DynamicTest> goodFixturePasses() {
    ...
    return new ContextMapRules(arch.layout())
        .rules().stream()
            .map(
                rule ->
