...
version = providers.gradleProperty("archunitSpringModulithVersion").getOrElse("0.1.0-SNAPSHOT")
...
extensions.configure<MavenPublishBaseExtension> {
    coordinates("dev.domaincentric", "dca-archunit-spring-modulith", version.toString())
    pom {
...
// A released dca-archunit-spring-modulith must not carry a snapshot of dca-archunit in its POM.
val archunitVersion = providers.gradleProperty("archunitVersion").getOrElse("0.1.0-SNAPSHOT")
val archunitSpringModulithVersion = version.toString()
tasks.matching { it.name.contains("MavenCentral") }
    .configureEach {
        doFirst {
            check(archunitSpringModulithVersion.endsWith("-SNAPSHOT") || !archunitVersion.endsWith("-SNAPSHOT")) {
                "dca-archunit-spring-modulith $archunitSpringModulithVersion would depend on dca-archunit $archunitVersion. " +
